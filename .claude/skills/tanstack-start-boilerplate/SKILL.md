...
**Q1. "기본 틀만 구성할까요?"**
- 선택지: `기본 틀만` (템플릿 구조 그대로, 홈 화면만 — 시안 없이 바로 기본 초안을 만든다) / `UX/UI까지 구현` (시안 제시 → 선택/수정 루프 → 화면 구현)
...
### 4단계 — 시안 제시 · 선택 · 수정 루프 (UX/UI 선택 시에만; 기본 틀이면 이 단계 전체를 건너뛴다)

선택된 프리셋의 화면들을 **와이어프레임 시안 2~3개**로 만들어 사용자가 고르게 한다. 시안마다 레이아웃 접근을 다르게 한다(예: 사이드바형 vs 탑바형, 카드 그리드형 vs 테이블 중심형) — 같은 걸 세 번 그리면 선택의 의미가 없다.

**시안 제작 도구 (환경에 따라 자동 선택):**
- `/design` 스킬이 사용 가능한 환경(Claude Code): `/design`을 호출해 하나의 캔버스에 시안별 아트보드를 나란히 배치한다. 각 아트보드에 시안 이름(A/B/C)을 라벨링한다.
- `/design`이 없는 환경(codex CLI 등 다른 에이전트, 스킬 미지원 환경): **자립형 HTML 파일**로 시안을 만든다. 시안 전체를 한 파일에 섹션으로 나란히 담고(시안 A/B/C 제목 포함), 외부 의존성 없는 인라인 CSS로 작성한 뒤 브라우저로 열어(`open <파일>`) 보여준다.

**와이어프레임 규칙 (두 도구 공통):**
- 회색조 박스·레이아웃·컴포넌트 배치·화면 흐름만. 색상, 브랜딩, 하이파이 비주얼은 절대 넣지 않는다.
- 이유: 실제 비주얼(컬러·타이포·컴포넌트 스타일)은 전부 Astryx 테마가 결정한다. 와이어프레임은 "무엇이 어디에 놓이는가"만 합의하는 도구다.

**선택 및 수정 루프:**
1. 시안을 보여준 뒤 AskUserQuestion으로 묻는다: 선택지 = 각 시안(A/B/C, 한 줄 특징 설명 포함) + `수정 요청` 옵션.
2. 사용자가 시안을 고르면 → 그 시안을 확정하고 5단계로 진행한다.
3. 사용자가 수정을 요청하면(또는 Other로 피드백을 적으면) → 피드백을 반영해 시안을 고쳐 다시 보여주고 1번으로 돌아간다. 사용자가 만족할 때까지 반복한다 — 횟수 제한 없음. 이 루프를 건너뛰고 임의로 시안을 확정하지 말 것: 여기서 합의된 와이어프레임이 5단계 구현의 명세가 된다.
4. 사용자가 명시적으로 "아무거나/알아서 해줘"라고 하면 가장 적합한 시안을 근거와 함께 선택하고 진행한다.

### 5단계 — UI 구현 (UX/UI 선택 시에만)

`references/astryx-rules.md`를 읽고 규칙을 준수하며, 4단계에서 사용자가 확정한 시안대로 구현한다. 핵심:
